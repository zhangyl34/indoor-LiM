...
const bool scan_pub_en = true, pcd_save_en = false;
// 发布经过运动畸变校正注册到 IMU 坐标系的点云数据
const bool dense_pub_en = true;
...
int main(int argc, char **argv) {
    ...
    nh.param<double>("filter_size_surf",filter_size_surf_min,0.5);  // 取 0.5，太小会崩溃
    nh.param<float>("cube_side_length", cube_len, 100.0);
    nh.param<float>("mapping/det_range", det_range, 300.0);
    nh.param<double>("mapping/gyr_cov", gyr_cov, 0.1);
    nh.param<double>("mapping/acc_cov", acc_cov, 0.1);
    nh.param<double>("mapping/b_gyr_cov", b_gyr_cov, 0.0001);
    nh.param<double>("mapping/b_acc_cov", b_acc_cov, 0.0001);
    nh.param<double>("preprocess/blind", param_blind, 0.01);
    nh.param<int>("preprocess/scan_line", param_scans, 16);
    nh.param<int>("point_filter_num", param_filters, 2);
    nh.param<vector<double>>("mapping/extrinsic_T", extrinT, vector<double>());
    nh.param<vector<double>>("mapping/extrinsic_R", extrinR, vector<double>());
    
    /* test*/
    nh.param<int>("preprocess/reflect_thresh", param_reflect, -1);
